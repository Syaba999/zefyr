...
class _HTMLNotusDecoder extends Converter<String, Delta> {
  Map<String, Map<String, dynamic>> toDeltaAttribute(Queue<Element> elemStack) {
    var deltaAttributeInline = <String, dynamic>{};
    var deltaAttributeLine = <String, dynamic>{};
  ...
  List<Map<String, dynamic>> toDeltaFormatList(Element element) {
    ...
    var map = <Element, Queue<Element>>{element: elemStack};
    while (deltaFormatList.map((e) => e is Element).contains(true)) {
      for (var i = 0; i < deltaFormatList.length; i++) {
        ...
        for (var j = 0; j < elem.children.length; j++) {
          ...
          final newElemStack = Queue<Element>.from(currentElemStack);
  ...
  Delta convert(String inputHTML) {
    var rootHTML = getRootHTML(inputHTML);
    ...
    var cursor = 0;
    for (var firstLayerTag in rootHTML.children) {
      var tagIdx = htmlString.indexOf(firstLayerTag.outerHtml, cursor);
    ...
    void removeRedundantNewLine(deltaList) {
      for (var i = 1; i < deltaList.length; i++) {
    ...
    void avoidConcatinatingPlainTextAndLineAttributes(deltaList) {
      ...
      for (var i = 1; i < deltaList.length - 1; i++) {
